DGRPX7 ; GENERATED FROM 'DG LOAD EDIT SCREEN 7' INPUT TEMPLATE(#420), FILE 2;10/26/23
 D DE G BEGIN
DE S DIE="^DPT(",DIC=DIE,DP=2,DL=1,DIEL=0,DU="" K DG,DE,DB Q:$O(^DPT(DA,""))=""
 ...
 I $D(^(.362)) S %Z=^(.362) S %=$P(%Z,U,12) S:%]"" DE(24)=%,DE(28)=% S %=$P(%Z,U,13) S:%]"" DE(25)=%,DE(29)=% S %=$P(%Z,U,14) S:%]"" DE(35)=%,DE(38)=%
 I $D(^(.385)) S %Z=^(.385) S %=$P(%Z,U,1) S:%]"" DE(42)=% S %=$P(%Z,U,2) S:%]"" DE(44)=%
...
X34 S:$P($G(^DPT(DFN,"VET")),U,1)="N" Y="@70212"
 Q
35 D:$D(DG)>9 F^DIE17,DE S DQ=35,DW=".362;14",DV="SX",DU="",DIFLD=.36235,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C35^DGRPX7"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 G RE
...
X36 S Y="@70213"
 Q
37 S DQ=38 ;@70212
38 D:$D(DG)>9 F^DIE17,DE S DQ=38,DW=".362;14",DV="SX",DU="",DIFLD=.36235,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C38^DGRPX7"
 S DU="Y:YES;N:NO;U:UNKNOWN;"
 S Y="NO"
 G Y
...
X40 I X="Y" S Y="@7022"
 Q
41 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=41 G A
42 D:$D(DG)>9 F^DIE17,DE S DQ=42,DW=".385;1",DV="DX",DU="",DIFLD=.3851,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C42^DGRPX7"
 S X="@"
 S Y=X
...
C42S S X="" G:DG(DQ)=X C42F1 K DB
 D ^DGRPX73
C42F1 Q
X42 S %DT="PX" D ^%DT S X=Y K:Y<1 X I $D(X) D H^DGUTL I $G(X)>0 S DFN=DA D DTCHK^DGRP7CP
 Q
 ;
43 D:$D(DG)>9 F^DIE17,DE S Y=U,DQ=43 G A
44 D:$D(DG)>9 F^DIE17,DE S DQ=44,DW=".385;2",DV="*P27.18'X",DU="",DIFLD=.3852,DLB=$$LABEL^DIALOGZ(DP,DIFLD)
 S DE(DW)="C44^DGRPX7"
 S DU="DG(27.18,"
 S X="@"
 S Y=X
...
C44S S X="" G:DG(DQ)=X C44F1 K DB
 D ^DGRPX74
C44F1 Q
X44 S:($G(X)]"")&(X'?1P.P) X=$$UPPER^DGUTL(X) S X=$$LOWER^DGUTL(X) S DIC("S")="I $P($G(^(0)),U)[""Original Award""" D ^DIC K DIC S DIC=DIE,X=+Y K:Y<0 X
 Q
 ;
